...
const goalPlannerResult = computed(() => {
  if (!calculator.value || calculator.value.id !== "goal-planner") {
    return null;
  }
  ...
  const futureGoal = years <= 0 ? currentGoal : currentGoal * Math.pow(1 + inflation / 100, years);
  const annualReturn = expectedReturn / 100;
  const monthlyRate = annualReturn === 0 ? 0 : annualReturn / 12;
  const months = Math.max(0, years * 12);
  const futureExisting = months === 0 ? existing : existing * Math.pow(1 + annualReturn, years);
...
<template>
  <main class="shell">
    <section v-if="calculator" class="detail">
      ...
      <div
        class="detail__panel"
        v-else-if="calculator.id === 'goal-planner' && goalPlannerResult"
      >
        <div class="calculator-layout">
          <form class="form" @submit.prevent>
            <SliderField
          ...
          <div class="calculator-results">
            <div class="results">
              ...
              <div class="result-card">
                ...
                <strong>{{
                  formatCurrency(goalPlannerResult.futureExisting)
                }}</strong>
              </div>
              <div class="result-card result-card--total">
                <span>Monthly investment needed</span>
                <strong>{{
                  formatCurrency(goalPlannerResult.monthlyInvestment)
                }}</strong>
              </div>
            </div>
